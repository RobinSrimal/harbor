...
use crate::data::{
    add_topic_member, create_connect_token, delete_pending_invite, ensure_peer_exists,
    get_all_topics, get_connection, get_current_epoch, get_current_epoch_key,
    get_peer_relay_info, get_pending_invite, get_topic, get_topic_admin, get_topic_members,
...
use super::protocol::{
    ...
    TopicInvite as WireTopicInvite, TopicJoin, TopicLeave,
};
...
pub async fn create_topic(service: &ControlService) -> ControlResult<TopicInvite> {
    use rand::{rngs::OsRng, Rng};
    let mut topic_id = [0u8; 32];
    OsRng.fill(&mut topic_id);

    let our_id = service.local_id();
    let relay_url = service.endpoint().addr().relay_urls().next().map(|u| u.to_string());
...
pub async fn join_topic(service: &ControlService, invite: TopicInvite) -> ControlResult<()> {
    let our_id = service.local_id();
    ...
    for member_info in invite.get_member_info() {
        ...
        if let Ok(client) = service.dial_peer(&member_info.endpoint_id, member_info.relay_url.as_deref()).await {
            let _ = client.rpc(join.clone()).await;
        }
    }
...
pub async fn list_topics(service: &ControlService) -> ControlResult<Vec<[u8; 32]>> {
    let db = service.db().lock().await;
...
pub async fn get_invite(service: &ControlService, topic_id: &[u8; 32]) -> ControlResult<TopicInvite> {
    let db = service.db().lock().await;
