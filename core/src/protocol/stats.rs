...
use super::types::{
    ProtocolError, ProtocolStats, IdentityStats, NetworkStats, DhtStats, TopicsStats,
    OutgoingStats, HarborStats, DhtBucketInfo, DhtNodeInfo, TopicDetails, TopicMemberInfo,
    TopicSummary,
};
...
impl Protocol {
    ...
    pub async fn get_stats(&self) -> Result<ProtocolStats, ProtocolError> {
        ...
        let (packets_stored, storage_bytes) = get_harbor_cache_stats(&db)
            .unwrap_or((0, 0));
    ...
    pub async fn get_dht_buckets(&self) -> Result<Vec<DhtBucketInfo>, ProtocolError> {
        ...
        for bucket_index in 0..=255u8 {
            ...
            if !entries.is_empty() {
                ...
                let nodes: Vec<DhtNodeInfo> = entries.into_iter().map(|entry| {
                    DhtNodeInfo {
    ...
    pub async fn get_topic_details(&self, topic_id: &[u8; 32]) -> Result<TopicDetails, ProtocolError> {
        ...
        let members: Vec<TopicMemberInfo> = members_info.into_iter().map(|m| {
            TopicMemberInfo {
