#!/bin/bash
#
# Scenario: Basic DM Stream Transport
#
# Tests the full DM stream pipeline (peer-to-peer, no topic):
# - DM stream request (signaling via DM)
...
scenario_dm_stream_basic() {
    log "=========================================="
    log "SCENARIO: Basic DM Stream Transport"
    log "=========================================="
    ...
    # Get endpoint IDs (no topic needed for DM streams)
    local NODE2_ID=$(api_get_endpoint_id 2)
    info "Node-2 endpoint: ${NODE2_ID:0:16}..."

    # Establish DM connection via Control protocol (required for STREAM_ALPN gate)
    log "Phase 2b: Establishing DM connection..."
    ...
    # Node-1 requests DM stream to Node-2
    log "Phase 3: Node-1 requesting DM stream to Node-2..."
    local req_response=$(api_dm_stream_request 1 "$NODE2_ID" "dm-test-stream")
    ...
    log "Phase 4: Waiting for stream establishment (20s)..."
    sleep 20

    # Check signaling events
    log "Phase 5a: Checking signaling events..."
    ...
    log "Phase 5b: Checking MOQ connection events..."
    ...
    # Publish test data
    log "Phase 6: Node-1 publishing test data..."
    local test_data=$(echo -n "HELLO-DM-STREAM" | xxd -p | tr -d '\n')
    local pub_result=$(api_stream_publish 1 "$REQUEST_ID" "$test_data")
    info "Publish result: $pub_result"

    # Wait for data to arrive
    log "Phase 7: Waiting for data transport (10s)..."
    sleep 10

    # Check for received data
    log "Phase 8: Checking data receipt on Node-2..."
    ...
    if [ "$n2_request" -gt 0 ]; then
        log "  ✅ Signaling: DM StreamRequest delivered to destination"
        passed=$((passed + 1))
    else
        warn "  ❌ Signaling: DM StreamRequest NOT received by destination"
    fi
    ...
    if [ $passed -eq $total ]; then
        log "✅ PASSED: Full DM stream pipeline working ($passed/$total)"
    elif [ $passed -gt 0 ]; then
        warn "⚠️  PARTIAL: Some checks passed ($passed/$total)"
    else
        warn "❌ FAILED: DM stream pipeline not working ($passed/$total)"
    fi

    log "=========================================="
    log "SCENARIO COMPLETE: Basic DM Stream Transport"
    log "=========================================="
}
